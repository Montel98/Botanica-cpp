...
Geometry Stem::generateGeometry(const LSystemParams &lParams, const StemNode* prevStem) {
	...
	if (lParams.connectParent && !prevStem) {
		...
		BufferAttributes& tip = stemBody.bufferAttributes;
		...
		for (int i = 0; i < vSteps; i++) {
			...
			int prevIndex = (uStepsBody * vSteps) + (uStepsTip * i);
			...
			body.getBufferAttribute<glm::vec3>("aVertexImmatureEnd").bufferData[uStepsBody*i] = bodyPrev.getBufferAttribute<glm::vec3>("aVertexImmatureEnd").bufferData[prevIndex];
			body.getBufferAttribute<glm::vec3>("aVertexStart").bufferData[uStepsBody*i] = bodyPrev.getBufferAttribute<glm::vec3>("aVertexStart").bufferData[prevIndex];
			...
			tip.getBufferAttribute<glm::vec3>("aVertexStart").bufferData[uStepsTip*i] = bodyPrev.getBufferAttribute<glm::vec3>("aVertexStart").bufferData[prevIndex];
		}
	}
